/**
 * AdminCivitatis — Gestión de reservas Civitatis
 * GuanaGO Super Admin · Abril 2026
 *
 * Datos: Airtable base appij4vUx7GZEwf5x
 *   - Tabla: Generador_vouchers  (vouchers existentes)
 *   - Tabla: Servicios Turisticos (dropdown de servicios)
 *
 * Flujo manual que reemplaza:
 *   Email Civitatis → leer datos → crear voucher en Airtable manualmente
 */

import React, { useEffect, useState, useCallback } from 'react';
import {
  ArrowLeft, Plus, Search, Loader2, CheckCircle2, Clock,
  XCircle, RefreshCw, Users, Calendar, Phone, Mail,
  FileText, MapPin, Hash, ChevronDown, X, Ticket,
} from 'lucide-react';
...
const AdminCivitatis: React.FC<AdminCivitatisProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white pb-20">
      ...
      {/* Header */}
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-4 flex items-center gap-3 sticky top-0 z-20">
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          <div>
            <h1 className="font-bold text-base leading-tight">Civitatis · Reservas</h1>
            <p className="text-xs text-gray-400">Gestión de reservas recibidas por Civitatis</p>
          </div>
        </div>
        <button
        ...
        <button
          onClick={openNewForm}
          className="flex items-center gap-2 bg-orange-600 hover:bg-orange-500 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors"
        >
          <Plus size={16} />
          Nueva reserva
        </button>
      ...
      {/* Info banner */}
      <div className="mx-4 mt-4 mb-2 bg-orange-950/40 border border-orange-800/40 rounded-xl px-4 py-3 text-sm text-orange-300">
        <p className="font-semibold mb-1">Flujo: Email Civitatis → Voucher en Airtable</p>
        <p className="text-xs text-orange-400/80">
          Cuando llega un email de confirmación de Civitatis, crea la reserva aquí.
          El ID de reserva tiene formato <span className="font-mono">A38417694</span>.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-500 py-20">
          ...
          <p className="text-sm">No hay reservas Civitatis aún.</p>
          ...
          {filtered.map(v => {
            ...
            return (
              <button
                ...
                className="w-full text-left bg-gray-900 border border-gray-800 hover:border-orange-700/60 rounded-2xl p-4 transition-colors"
              >
                ...
                <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                  ...
                  {v.pax && (
                    <span className="flex items-center gap-1">
                      <Users size={11} /> {v.pax} pax
                    </span>
                  )}
                  {v.puntoEncuentro && (
                    <span className="flex items-center gap-1">
                      <MapPin size={11} /> {v.puntoEncuentro}
                    </span>
                  )}
                </div>
              </button>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 sticky top-0 bg-gray-900">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-bold text-base">Nueva reserva Civitatis</h2>
              </div>
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block font-medium">
                  <Hash size={11} className="inline mr-1" />Reserva # Civitatis
                </label>
                <input
              ...
              <div>
                ...
                {form.tourName && !form.tourId && (
                  <input
                    value={form.tourName}
                    onChange={e => setField('tourName', e.target.value)}
                    placeholder="Nombre manual del tour"
                    className="mt-2 w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500"
                  />
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {ESTADOS.map(e => {
                    const cfg: EstadoCfg = ESTADO_CFG[e] || { label: e, bg: 'bg-gray-800', text: 'text-gray-300', icon: null };
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Notas del cliente, necesidades especiales…"
              ...
              {saveOk && (
                <div className="bg-green-900/40 border border-green-700/50 rounded-xl px-4 py-3 text-sm text-green-300 flex items-center gap-2">
                  <CheckCircle2 size={15} /> Reserva creada en Airtable
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-3xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            <div className="px-5 py-4 space-y-3 text-sm">
              ...
              {/* Estado */}
              {(() => {
                const cfg: EstadoCfg = ESTADO_CFG[selectedVoucher.estado] || { label: selectedVoucher.estado, bg: 'bg-gray-800', text: 'text-gray-300', icon: null };
                return (
                ...
                <div className="bg-gray-800 rounded-xl px-4 py-3 space-y-1">
                  ...
                  {selectedVoucher.telefono && (
                    <p className="flex items-center gap-2 text-xs">
                      ...
                      <a href={`tel:${selectedVoucher.telefono}`} className="text-blue-400 hover:underline">
                        {selectedVoucher.telefono}
                      </a>
                    </p>
                  )}
                  {selectedVoucher.email && (
                    <p className="flex items-center gap-2 text-xs">
                      ...
                      <a href={`mailto:${selectedVoucher.email}`} className="text-blue-400 hover:underline">
                        {selectedVoucher.email}
                      </a>
                    </p>
                  )}
              ...
              {selectedVoucher.observaciones && (
                <div className="bg-amber-950/30 border border-amber-800/30 rounded-xl px-4 py-3">
                  <p className="text-xs text-amber-400 font-medium mb-1">Observaciones especiales</p>
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-gray-800 hover:bg-gray-700 text-sm font-medium transition-colors"
                >
                  Cerrar
                </button>
              </div>
