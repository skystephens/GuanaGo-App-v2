...
const SEED_SECCIONES: SeccionControl[] = [
  ...
  {
    id: 'experiencia_b2c', titulo: 'Experiencia del Visitante',
    subtitulo: 'Itinerario inteligente, reseñas, recomendaciones personalizadas y perfil de gustos',
    icono: 'Map', color: 'text-emerald-400', gradiente: 'from-emerald-900 to-emerald-800',
    creadaEn: new Date().toISOString(),
    tareas: [
      { id: 'exp-01', titulo: 'Itinerario guardado en Firestore', descripcion: 'MyItinerary.tsx conectado a Firestore: usuarios/{uid}/itinerarios/{id}. Guardar, editar, eliminar días y actividades. Persiste entre sesiones y dispositivos.', prioridad: 'critica', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'exp-02', titulo: 'Onboarding: perfil de gustos del visitante', descripcion: 'Al registrarse, el turista elige intereses (Aventura, Cultura, Gastronomía, Relax, Nocturna), nivel de actividad y presupuesto. Se guarda en Firestore + Airtable Preferencias_Visitante.', prioridad: 'critica', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'exp-03', titulo: 'Reseñas verificadas por reserva', descripcion: 'Después de reserva completada: visitante puede dejar reseña (1-5 estrellas + comentario). Solo usuarios con reserva confirmada. Alimenta calificación promedio del servicio en Airtable.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'exp-04', titulo: 'Recomendaciones personalizadas con IA', descripcion: '/api/recommendations/generate: agentService analiza preferencias + historial + fechas → array de servicios recomendados con motivo. Guarda en Firestore recomendaciones/{uid}.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'exp-05', titulo: 'Sección "Para ti" en Home', descripcion: 'Bloque dinámico en Home con servicios recomendados por IA según perfil del visitante. Si sin perfil: muestra los más reservados. Se rota en cada visita.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'exp-06', titulo: 'Compartir itinerario (link público)', descripcion: 'Itinerario con opción "Compartir": URL pública /itinerario/:id visible sin login. Ideal para WhatsApp con acompañantes de viaje.', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'exp-07', titulo: 'Push: recordatorio 24h antes de actividad', descripcion: 'FCM: 24h antes de actividad en itinerario → push con nombre, hora, ubicación e info del socio. Requiere FCM configurado y permiso del usuario.', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'exp-08', titulo: 'Mapa de ruta del día en itinerario', descripcion: 'Botón "Ver ruta del día" en MyItinerary: Mapbox muestra actividades ordenadas con línea de ruta y estimado de tiempo entre puntos.', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
    ],
  },
  {
    id: 'airtable_datos', titulo: 'Airtable — Estructura de Datos',
    subtitulo: 'Tablas nuevas para experiencias, reseñas, preferencias y métricas de negocio',
    icono: 'BookOpen', color: 'text-lime-400', gradiente: 'from-lime-900 to-lime-800',
    creadaEn: new Date().toISOString(),
    tareas: [
      { id: 'air-01', titulo: 'Crear tabla Itinerarios_GG', descripcion: 'Campos: ID_Itinerario, Usuario_ID (link Leads), Nombre_Viaje, Fecha_Inicio, Fecha_Fin, Estado (activo/pasado/borrador), Total_Servicios, Compartible (checkbox), Notas_Personales.', prioridad: 'critica', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'air-02', titulo: 'Crear tabla Preferencias_Visitante', descripcion: 'Campos: Usuario_ID (link Leads), Tipos_Favoritos (multi: Aventura/Cultura/Gastronomía/Relax/Nocturna), Presupuesto_Diario_COP, Grupo (solo/pareja/familia/amigos), Nivel_Actividad, Restricciones, Origen_Ciudad.', prioridad: 'critica', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'air-03', titulo: 'Crear tabla Reseñas_GG', descripcion: 'Campos: Servicio_ID (link ServiciosTuristicos_SAI), Usuario_ID (link Leads), Calificacion (1-5), Comentario, Fecha, Verificada (tiene reserva confirmada), Respuesta_Aliado. Formula promedio en ServiciosTuristicos.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'air-04', titulo: 'Crear tabla Metricas_Diarias (KPI snapshot)', descripcion: 'Campos: Fecha, Reservas_Nuevas, Revenue_COP, Usuarios_Nuevos, Chats_IA, Cotizaciones_Generadas, Top_Servicio, DAU. Se llena cada noche via Make.com desde Firestore analytics.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'air-05', titulo: 'Ampliar ServiciosTuristicos_SAI', descripcion: 'Nuevos campos: Categorias_Experiencia (multi: Aventura/Cultura/Gastronomía/Relax/Nocturna), Tags_Raizal (checkbox), Calificacion_Promedio (fórmula), Total_Reservas (count), Destacado (checkbox), Orden_Display (número).', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'air-06', titulo: 'Ampliar tabla Reservas', descripcion: 'Nuevos campos: Canal (B2C App/B2B Agencia/Directo), Fuente_IA (si vino de recomendación del agente), Conversion_Desde (cotización/itinerario/búsqueda), Calificada (checkbox si dejó reseña).', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'air-07', titulo: 'Make.com: sync Firestore analytics → Metricas_Diarias', descripcion: 'Scenario nocturno: lee analytics de Firestore + Reservas del día → crea registro en Metricas_Diarias. Alimenta gráficas del AdminDashboard CEO.', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
    ],
  },
  {
    id: 'backend_faltante', titulo: 'Backend — APIs Pendientes',
    subtitulo: 'Endpoints para itinerario, reseñas, recomendaciones, métricas y reservas reales',
    icono: 'Zap', color: 'text-blue-400', gradiente: 'from-blue-900 to-blue-800',
    creadaEn: new Date().toISOString(),
    tareas: [
      { id: 'api-01', titulo: 'CRUD /api/itinerary', descripcion: 'GET /:userId → lista itinerarios. POST → crear. PUT /:id → actualizar actividades. DELETE /:id. Persiste en Firestore usuarios/{uid}/itinerarios. Requiere Firebase Auth.', prioridad: 'critica', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'api-02', titulo: 'POST /api/reviews + GET /api/reviews/:servicioId', descripcion: 'POST valida que usuario tenga reserva confirmada antes de crear reseña. GET trae reseñas con paginación. PATCH /api/reviews/:id/reply → respuesta del aliado al comentario.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'api-03', titulo: 'GET/POST /api/recommendations', descripcion: 'GET /:userId trae recomendaciones guardadas. POST /generate: agentService analiza perfil + historial → devuelve array de servicioIds recomendados con motivo. Guarda en Firestore.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'api-04', titulo: 'GET /api/admin/metrics (KPIs reales)', descripcion: 'Lee Firestore analytics + Airtable Metricas_Diarias. Devuelve: DAU, MAU, reservas/mes, revenue/mes, top 5 servicios, top 5 aliados, reseñas recientes sin respuesta. Para AdminDashboard.', prioridad: 'critica', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'api-05', titulo: 'Conectar /api/quotations a Airtable real', descripcion: 'Reemplazar mock data en quotations.js con llamadas reales a tabla CotizacionesGG de Airtable. GET lista real, POST crea registro, PATCH actualiza estado. Compatible con GuiaSAI Business.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'api-06', titulo: 'POST /api/reservations: dual write Airtable + Firestore', descripcion: 'Al confirmar reserva: write en Airtable Reservas (fuente de verdad) + Firestore reservas/{id} (runtime). Disparar logEvent(). Push al socio via FCM.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'api-07', titulo: 'CRUD /api/preferences/:userId', descripcion: 'GET devuelve perfil de gustos. POST crea preferencias en onboarding. PUT actualiza. Guarda en Firestore + Airtable Preferencias_Visitante para personalización de recomendaciones.', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
    ],
  },
  {
    id: 'admin_metricas', titulo: 'Admin CEO — Métricas & Gráficas',
    subtitulo: 'AdminDashboard con KPIs reales, gráficas de negocio y alertas operativas',
    icono: 'BarChart2', color: 'text-rose-400', gradiente: 'from-rose-900 to-rose-800',
    creadaEn: new Date().toISOString(),
    tareas: [
      { id: 'met-01', titulo: 'Conectar AdminDashboard a métricas reales', descripcion: 'AdminDashboard.tsx usa mock. Reemplazar con /api/admin/metrics. Mostrar: reservas hoy, revenue mes, usuarios DAU, chats IA. Comparativa vs semana anterior con flecha indicadora.', prioridad: 'critica', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'met-02', titulo: 'Gráfica de línea: reservas diarias 30 días', descripcion: 'Line chart (recharts). Datos desde Airtable Metricas_Diarias. Muestra reservas por día vs meta diaria. Color emerald GuanaGO. Click en punto muestra detalle del día.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'met-03', titulo: 'Gráfica de barras: ingresos semanales', descripcion: 'Bar chart: revenue COP por semana, últimas 8 semanas. Header muestra total del mes. Datos de Airtable Metricas_Diarias agrupados por semana ISO.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'met-04', titulo: 'Donut: distribución por tipo de servicio', descripcion: 'Donut chart: % de reservas por tipo (Tour/Hotel/Taxi/Paquete/Caribbean Night). Datos de Airtable Reservas con groupBy Tipo. Click en segmento filtra lista de servicios debajo.', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'met-05', titulo: 'Rankings: top servicios y top aliados', descripcion: 'Tabla top 5 servicios por reservas + top 5 aliados por revenue generado. Datos Airtable Reservas con groupBy. Click navega al detalle en AdminServices o AdminSocios.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'met-06', titulo: 'Panel de alertas operativas en tiempo real', descripcion: 'Sección de alertas: aliados sin actividad 7+ días, servicios sin disponibilidad, cotizaciones sin respuesta 48h. Badge con conteo en el nav del admin. Actualización automática cada 5min.', prioridad: 'alta', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'met-07', titulo: 'Reseñas recientes pendientes de respuesta', descripcion: 'Lista de últimas 10 reseñas sin respuesta del aliado. Botón "Responder" abre modal. Respuesta se guarda en Firestore y Airtable Reseñas_GG.', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
      { id: 'met-08', titulo: 'Reporte ejecutivo semanal por email', descripcion: 'Lunes 8am: Make.com envía email al CEO con resumen: reservas semana, revenue, usuarios nuevos, top servicio, alertas activas. Template HTML con colores GuanaGO via Resend.com.', prioridad: 'media', estado: 'pendiente', creadaEn: new Date().toISOString() },
    ],
  },
];
